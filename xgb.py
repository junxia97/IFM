...
def hyper_runing(subtask):
    ...
    sub_dataset.iloc[:, 1:] = sub_dataset.iloc[:, 1:].apply(standardize, axis=0)
...
if len(tasks) == 1:
    ...
    args = {'data_label': dataset_label, 'metric': 'auc_prc' if task_type == 'cla' else 'mae', 'model': 'XGB'}
    print('{}_{}: the mean {} for the training set is {:.3f} with std {:.3f}'.format(args['data_label'], args['model'],
                                                                                     args['metric'], np.mean(
            stat_res[stat_res['set'] == 'tr'][args['metric']]), np.std(
            stat_res[stat_res['set'] == 'tr'][args['metric']])))
    ...
    print('{}_{}: the mean {} for the test set is {:.3f} with std {:.3f}'.format(args['data_label'], args['model'],
            ...
            stat_res[stat_res['set'] == 'te'][args['metric']]), np.std(
            stat_res[stat_res['set'] == 'te'][args['metric']])))
    args = {'data_label': dataset_label, 'metric': 'auc_roc' if task_type == 'cla' else 'r2', 'model': 'XGB'}
    print('{}_{}: the mean {} for the training set is {:.3f} with std {:.3f}'.format(args['data_label'], args['model'],
                                                                                     args['metric'], np.mean(
            stat_res[stat_res['set'] == 'tr'][args['metric']]), np.std(
            stat_res[stat_res['set'] == 'tr'][args['metric']])))
...
# multi-tasks
    ...
    args = {'data_label': dataset_label, 'metric': 'auc_roc' if dataset_label != 'muv' else 'auc_roc', 'model': 'XGB'}
    ...
    print('{}_{}: the mean {} for the test set is {:.3f} with std {:.3f}'.format(args['data_label'], args['model'],
                                                                                 ...
                                                                                 np.std(te_acc)))
    args = {'data_label': dataset_label, 'metric': 'auc_prc' if dataset_label != 'muv' else 'auc_prc', 'model': 'XGB'}
    tr_acc = np.zeros(repetitions)
    va_acc = np.zeros(repetitions)
    te_acc = np.zeros(repetitions)
    for subtask in tasks:
