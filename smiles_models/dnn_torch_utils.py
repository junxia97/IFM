...
class Meter(object):
    ...
    def roc_precision_recall_score(self):
        ...
        for task in range(n_tasks):
            #task_w = mask[:, task]
            task_y_true = y_true[:, task].numpy()#[task_w != 0].numpy()
            task_y_pred = y_pred[:, task].numpy()##[task_w != 0].numpy()
    ...
    def l1_loss(self, reduction):
        ...
        for task in range(n_tasks):
            #task_w = mask[:, task]
            task_y_true = y_true[:, task].numpy()#[task_w != 0]
            task_y_pred = y_pred[:, task].numpy()#[task_w != 0]
