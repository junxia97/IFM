...
class TrfmSeq2seq(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, n_layers):
        ...
        self.pe = PositionalEncoding(hidden_size, 0.2)
    ...
    def _encode(self, src):
        ...
        # mean, max, first*2
        return torch.cat([torch.mean(output, dim=0), torch.max(output, dim=0)[0], output[0,:,:], penul[0,:,:]], dim=1)
...
class e_TrfmSeq2seq(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, n_layers, dropout=0.1,reg=False,outputs=1):
        ...
        self.trfm = nn.Transformer(d_model=hidden_size, nhead=4, 
        num_encoder_layers=n_layers, num_decoder_layers=n_layers, dim_feedforward=hidden_size)
        #self.po = nn.Linear(hidden_size, out_size)
        
        #reg or cla
        if reg:
            self.out = nn.Linear(hidden_size*4, 1)
        else:
            self.out = nn.Linear(hidden_size*4, outputs)
