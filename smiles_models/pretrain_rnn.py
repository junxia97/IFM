...
class RNNSeq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5): # (T,B)
        ...
        outputs = Variable(torch.zeros(max_len, batch_size, vocab_size)).to(device) # (T,B,V)
...
class e_RNNSeq2Seq(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, n_layers, dropout=0.1,reg=False,outputs=1):
        super(e_RNNSeq2Seq, self).__init__()
        self.encoder = Encoder(in_size, hidden_size, hidden_size, n_layers,dropout=dropout)
        self.decoder = Decoder(hidden_size, hidden_size, out_size, n_layers,dropout=dropout)
        if reg:
            self.out = nn.Linear(hidden_size, 1)
        else:
            self.out = nn.Linear(hidden_size, outputs)
    def forward(self, src): # (T,B)
        return self.out(self._encode(src))
        '''
        batch_size = src.size(1)
        max_len = trg.size(0)
        vocab_size = self.decoder.output_size
    ...
    def _encode(self, src):
        ...
        print(hidden.shape)
        return hidden
        #return torch.cat(hidden[2:],dim=1) #(B,4H)
...
def parse_arguments():
    ...
    parser.add_argument('--train_data', type=str, default='data/chembl_24.csv', help='train corpus (.csv)')
    parser.add_argument('--test_data', type=str, default='data/chembl_24.csv', help='test corpus (.csv)')
    ...
    parser.add_argument('--n_worker', '-w', type=int, default=2, help='number of workers')
...
def evaluate(model, val_loader, vocab):
    ...
    for b, data in enumerate(val_loader):
        sm1, sm2 = torch.t(data.to(device)), torch.t(data.to(device)) # (T,B)
        #print(sm1)
        #print(sm2)
        with torch.no_grad():
...
def main():
    ...
    model = RNNSeq2Seq(len(vocab),hidden_size, len(vocab),n_layers=3).to(device)
    ...
    for e in range(1, args.n_epoch):
        for b,data in tqdm(enumerate(train_loader)):
            ...
            #print(data.shape)
            #print(data)
            #sm1, sm2 = torch.t(data[0].cuda(7)), torch.t(data[1].cuda(7)) # (T,B)
            sm1, sm2 = torch.t(data.to(device)), torch.t(data.to(device)) # (T,B)
            #sm1 = sm1.unsqueeze(0)
            #sm2 = sm2.unsqueeze(0)
            optimizer.zero_grad()
