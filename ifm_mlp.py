...
import numpy as np
import pandas as pd
from ednn_utils import Meter, MyDataset, EarlyStopping, MyDNN, SIM_DNN, LE_DNN, LSIM_DNN, IFM_DNN, GM_DNN, collate_fn, set_random_seed
...
def main():
    ...
    parser.add_argument('--embed', type=str, default = 'GM', help='Embedding method: None, LE, LSIM, GM, IFM, SIM.')
    ...
    def hyper_opt(hyper_paras):
        ...
        if args.embed == 'None':
        ...
        elif args.embed == 'LSIM':
            my_model = LSIM_DNN(inputs=inputs, hidden_units=hidden_units, d_out=hyper_paras['d_out'] + 1, sigma=hyper_paras['sigma'], dp_ratio=hyper_paras['dropout'], outputs=len(tasks), reg=args.reg)
        elif args.embed == 'IFM':
            my_model = IFM_DNN(inputs=inputs, hidden_units=hidden_units, d_out=hyper_paras['d_out'] + 1, sigma=hyper_paras['sigma'], dp_ratio=hyper_paras['dropout'], first_omega_0=hyper_paras['omega0'], hidden_omega_0=hyper_paras['omega1'], outputs=len(tasks), reg=args.reg)
        elif args.embed == 'GM':
            ...
            my_model = SIM_DNN(inputs=inputs, hidden_units=hidden_units, d_out=hyper_paras['d_out'] + 1, sigma=hyper_paras['sigma'] + 1, dp_ratio=hyper_paras['dropout'], outputs=len(tasks), reg=args.reg)
        else:
            raise ValueError("Invalid Embedding Name")
    ...
    if args.embed == 'None':
        ...
        best_model = LSIM_DNN(inputs=inputs, hidden_units=opt_hidden_units, d_out=opt_res['d_out']+ 1, sigma=opt_res['sigma'], dp_ratio=opt_res['dropout'], outputs=len(tasks), reg=args.reg)
    elif args.embed == 'IFM':
        best_model = IFM_DNN(inputs=inputs, hidden_units=opt_hidden_units, d_out=opt_res['d_out'] + 1, sigma=opt_res['sigma'], dp_ratio=opt_res['dropout'], first_omega_0=opt_res['omega0'], hidden_omega_0=opt_res['omega1'], outputs=len(tasks), reg=args.reg)
    elif args.embed == 'GM':
        ...
        best_model = SIM_DNN(inputs=inputs, hidden_units=opt_hidden_units, d_out=opt_res['d_out']+ 1, sigma=opt_res['sigma']+1, dp_ratio=opt_res['dropout'], outputs=len(tasks), reg=args.reg)
    else:
        raise ValueError("Invalid Embedding Name")
